...
describe('One provider by property (unit)', () => {
  ...
  it('Work with one provider', async () => {
    ...
    @Injectable()
    class P {
      @CustomInject<CProvider>(C_PROVIDER, { propertyName: 'type' })
  ...
  it('Error if provider not set', async () => {
    ...
    @Injectable()
    class P {
      @CustomInject<CProvider>(C_PROVIDER1, { propertyName: 'type' })
      provider!: CProvider['type'];
    }
    const module = await Test.createTestingModule({
  ...
  it('Async default value if provider not set', async () => {
    ...
    await app.init();
    const p = app.get<P>(P);
    expect(p.provider).toEqual('async');
    await app.close();
  });
